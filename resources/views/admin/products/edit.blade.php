...
@section('content')
                    ...
                    <div class="image-preview mt-3" id="image-preview">
                        <div class="preview-header d-flex justify-content-between align-items-center">
                            ...
                            <button type="button" class="btn btn-sm btn-outline-danger" id="remove-image">
                                <i class="fas fa-times"></i> Change Image
                            </button>
                        </div>
                        <div class="preview-image mt-2">
                            @if($product->image)
                                <img id="preview-img" src="{{ asset('images/'.$product->image) }}" alt="Current Image" class="img-fluid rounded" style="max-height: 200px;">
                                <small class="text-muted mt-1">Current: {{ $product->image }}</small>
                            @else
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function previewImage(file) {
            ...
            reader.onload = function(e) {
                ...
                if (!infoDiv) {
                    ...
                    previewImg.parentNode.appendChild(infoDiv);
                }
                infoDiv.textContent = `New: ${file.name} (${sizeKB} KB)`;
            };
        ...
        function resetImageUpload() {
            ...
            @if($product->image)
                previewImg.src = '{{ asset("images/" . $product->image) }}';
                const infoDiv = document.getElementById('file-info');
                if (infoDiv) {
                    infoDiv.textContent = 'Current: {{ $product->image }}';
                }
            @else
                const infoDiv = document.getElementById('file-info');
                if (infoDiv) {
                    infoDiv.textContent = 'No image uploaded';
                }
            @endif
